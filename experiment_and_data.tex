...
The \red{new} data \red{presented in this work were} collected in 2006 and 2007 with the H{\sc ermes}
...
target internal to the \red{H\sc{era}} lepton storage ring at \red{D\sc{esy}}. The integrated
...
(\phi)],
\end{equation}
where \red{$P_\ell$ is the beam polarisation, }$\mathcal{L}$ is the integrated luminosity, $\eta$ is the detection
...
Fourier series \red{that would describe the fitted distribution
...
are included as both a consistency check for \red{any off-phase}
extraneous harmonics \red{in the data} and as a test of the
normalisation of the fit. These terms are expected to be
\red{consistent with zero and are found to be so}.
...
\red{Event w}eights are introduced in the fitting procedure to account for
...
combination of complex Compton Form Factors (CFF). A CFF is a convolution of a GPD with a hard scattering kernel. Contributions of GPDs to the cross section may be relatively suppressed by powers of $1/Q$, the origin of which may be kinematic in nature or due to the twist level of the GPD. The access from each asymmetry amplitude to a GPD is called twist-$n$ depending upon the part of the twist-decomposed cross section in which the amplitude appears. Leading twist is twist-2. Typically, the contribution of a twist-$n$ GPD, and hence the corresponding CFF, is suppressed by $\mathcal{O}(1/Q^{n-2})$. Table~\ref{tab_amplitudes} presents the asymmetry amplitudes extracted in this analysis and the related Fourier coefficients, dominant $\mathcal{C}$-function, twist-level and level of suppression.
...
\end{equation}
\end{center}
where the \red{definition of the} kinematic factor $k$ \red{is found
...
neglected in Eq.~\ref{Eq_cunp} \red{(in first approximation)} with respect to $\mathcal{H}$ since they
...
which is bilinear in CFFs. However, this \red{twist-3} amplitude is inherently small at HERMES kinematic conditions due to the size of the $s_{1}^{\textrm{DVCS}}$ Fourier coefficient compared to the contribution from the $c_{n}^{\textrm{BH}}$ coefficients in the denominator of Eq.~\ref{e:aludvcs}. As a result of
the more complicated dependence on the CFFs and this suppression, it
\red{is} more difficult to \red{constrain} GPDs
...
where $\alpha_{\textrm{S}}$ is the \red{strong coupling} constant.
\begin{table}
\begin{center}
\resizebox{\textwidth}{!} {
...
be extracted from the available data set, the related Fourier coefficients,
 ...
 \red{relative to the leading amplitudes at H{\sc ermes} kinematic conditions}~\cite{Bel02b}.}
