...
approximately 246\,pb$^{-1}$ and 1460\,pb$^{-1}$, with average beam polarisations of \red{$0.303$ and $0.392$.} The procedure used to select events is similar to that used in ref.~\cite{Air09}. A brief summary of this procedure is outlined in the following; consult
...
rest frame. The polar angle between the \red{directions of the} virtual and real photons was required to
...
energy resolution of the H{\sc ermes} \red{spectrometer (mainly the calorimeter)}. Here, $q$ is the
...
\red{The data sample in the exclusive region contains events \red{not only} involving production of real photons in which the proton remains intact, \red{but also}
events involving the excitation of the target proton to a $\Delta^+$
resonant state (``associated production''). The recoiling proton is not detected and the calorimeter resolution does not allow separation of \red{all of} the latter events from the rest of the data sample.
...
cross section for an unpolarised target \red{summed} over both beam charges and
...
$\mathcal{A}_{\textrm{C}}$ asymmetry are \red{motivated by the
  physical processes under investigation}. The other terms
are included both \red{as} a consistency check for any off-phase
...
coefficients $s_{n,\textrm{unp}}^{W}$ and $c_{n,\textrm{unp}}^{I}$ from the interference and DVCS terms in eqs.~\ref{e:alui}-\ref{e:ac}. The asymmetry amplitudes
...
The DVCS asymmetry amplitude $A^{\sin\phi}_{\textrm{LU,DVCS}}$ \red{receives} a
contribution from the $\mathcal{C}_{\textrm{unp}}^{\textrm{DVCS}}$-function,
which is bilinear in CFFs. However, this twist-3 amplitude is inherently small \red{in} HERMES kinematic conditions due to the size of the $s_{1}^{\textrm{DVCS}}$ Fourier coefficient compared to the contribution from the $c_{n}^{\textrm{BH}}$ coefficients in the denominator of Eq.~\ref{e:aludvcs}. As a result of the more complicated dependence on the CFFs and this suppression, it
...
\red{The leading-twist asymmetry amplitudes are $A_{\textrm{C}}^{\cos(0\phi)}$, $A_{\textrm{C}}^{\cos\phi}$ and $A_{\textrm{LU,I}}^{\sin\phi}$, which are proportional to the fourier coefficients defined in eqs.~\ref{eq:c0}---\ref{eq:c1}. Whilst all of these amplitudes receive contributions from $\mathcal{C}_{\textrm{unp}}^{\textrm{I}}$, $c_{0,\textrm{unp}}^{\textrm{I}}$ is kinematically suppressed in comparison to $c_{1,\textrm{unp}}^{\textrm{I}}$, so $A_{\textrm{C}}^{\cos\phi}$ and $A_{\textrm{LU,I}}^{\sin\phi}$ are expected to have the largest magnitude in H{\sc ermes} kinematic conditions.

Although strictly dependent on higher-twist quantities, the asymmetry amplitudes $A_{\textrm{LU},\textrm{I}}^{\sin(2\phi)}$ and $A^{\cos(2\phi)}_{\textrm{C}}$ can also be expressed as having a dependence on $\mathcal{C}_{\textrm{unp}}^{\textrm{I}}$ using the Wandzura-Wilzcek approximation. Neglecting antiquark-gluon-quark contributions, these higher-twist dependent amplitudes can be considered as being kinematically suppressed compared to the leading-twist amplitudes.}
...
related Fourier coefficients, dominant $\mathcal{C}$-function, and the twist-level at which the contributing GPDs enter.
\begin{table}
\begin{center}
\resizebox{\textwidth}{!} {
