...
approximately 246\,pb$^{-1}$ and 1460\,pb$^{-1}$\blue{,} with average beam polarisation\blue{s} of 30.3\,\% and 39.2\% \cite{Ben01}. The procedure used to select events is \blue{similar} to that used in Ref.~\cite{Air09}.
...
extract the asymmetry amplitudes in \blue{each} kinematic bin of $-t$, $x_{\textrm{B}}$ and $Q^{2}$.
This method, described in Ref.~\cite{Air08}, fits the expected
\blue{azimuthal} distribution function to the data without introducing binning effects in $\phi$.
...
coefficients $s_{n}^{W}$ and $c_{n}^{V}$ from the interference and DVCS terms in Eqs.~\ref{e:alui}-\ref{e:ac}. The asymmetry amplitudes may also be affected by the lepton progopagators and the other $\phi$-dependent terms in the denominators in Eqs.~\ref{e:alui}-\ref{e:ac}. The  $s_{n}^{W}$ and $c_{n}^{V}$ Fourier coefficients \blue{depend on}  ``$\mathcal{C}$-functions''\blue{,} each \blue{of which} is a combination of complex Compton Form Factors. \blue{A CFF is a convolution of a GPD with a hard scattering kernel}. \blue{Contributions of GPDs to the cross section may be relatively suppressed by powers of $1/Q$, the origin of which may be kinematic in nature or due to the twist level of the GPD.} The access from each asymmetry amplitude to a GPD is called twist-$n$ depending upon the part of the twist-decomposed cross-section in which the amplitude appears. Leading twist is twist-2. Typically, the contribution of a \blue{twist-$n$} GPD, and hence the corresponding CFF, is suppressed by $\mathcal{O}(1/Q^{n-2})$. \blue{Table~\ref{tab_amplitudes} presents t}he asymmetry amplitudes extracted in this analysis and the related Fourier coefficients, dominant $\mathcal{C}$-function, twist-level and levels of suppression\blue{.}

At H{\sc ermes} kinematic\blue{ conditions}, the leading-twist beam-helicity asymmetry amplitude is $A_{\textrm{LU,I}}^{\sin\phi}$ and the leading beam-charge asymmetry amplitude is $A^{\cos\phi}_{\textrm{C}}$. These asymmetry amplitudes for an unpolarised target are sensitive respectively to Fourier coefficients $s_{1,\textrm{unp}}^{\textrm{I}}$ and $c_{1,\textrm{unp}}^{\textrm{I}}$. Both of these Fourier coefficients have a dominant contribution from the $\mathcal{C}_{\textrm{unp}}^{\textrm{I}}$-function:
\begin{center}
\begin{equation}
...
conditions (where $x_{\textrm{B}}$ and $\frac{t}{4M^2}$ are of order 0.1), \blue{to first approximation} the
...
Hence, \blue{in this} approximation, the behaviour of
...
upon this $\mathcal{C}$-function. However, the $A^{\cos(0\phi)}_{\textrm{C}}$ \blue{amplitude} is kinematically suppressed compared to the $A^{\cos\phi}_{\textrm{C}}$ amplitude, \blue{and} the  $A_{\textrm{LU,I}}^{\sin(2\phi)}$ and $A^{\cos(2\phi)}_{\textrm{C}}$ \blue{amplitudes} enter the cross section at a higher
twist level (see Table~\ref{tab_amplitudes}).

The DVCS asymmetry amplitude $A^{\sin\phi}_{\textrm{LU,DVCS}}$ has a dominant
contribution from the $\mathcal{C}_{\textrm{unp}}^{\textrm{DVCS}}$ function,
which is bilinear in CFFs. \blue{However, the amplitude is suppressed by the larger BH contribution in the denominator of Eq.~\ref{e:aludvcs}. As a result of
the more complicated dependence on the CFFs and th\blue{is} suppression, it
will be more difficult to extract useful information for the constraint of GPDs
from the measurement of $A^{\sin\phi}_{\textrm{LU,DVCS}}$ than f\blue{rom} the
...
in this function are \blue{of} leading twist, they relate to gluon helicity\blue{-}flip GPDs and are \blue{thus} suppressed by \blue{$\alpha_{\textrm{S}}/\pi$}.
\begin{table}[H]
\begin{center}
\resizebox{16cm}{!} {
 \begin{tabular}{|c|c|c|c|c|}
\hline
Asymmetry Amplitude & Fourier Coefficient& Dominant CFF Dependence & Twist Level  & \blue{Relative} Suppression \\
\hline
\hline
...
be extracted from the available data set, the related Fourier coefficients,
 the $\mathcal{C}$-functions, the twist level and the suppression \blue{at H{\sc ermes} kinematic conditions}~\cite{Bel02b}.}
