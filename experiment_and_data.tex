...
energy resolution of the H{\sc ermes} spectrometer (mainly the calorimeter). Here, $q$ is the
...
resonant state (``associated production''). The recoiling proton is not detected and the calorimeter resolution does not allow separation of all of the latter events from the rest of the data sample.
...
The leading-twist asymmetry amplitudes are $A_{\textrm{C}}^{\cos(0\phi)}$, $A_{\textrm{C}}^{\cos\phi}$ and $A_{\textrm{LU,I}}^{\sin\phi}$, which are proportional to the fourier coefficients defined in eqs.~\ref{eq:c0}---\ref{eq:c1}. Whilst all of these amplitudes receive contributions from $\mathcal{C}_{\textrm{unp}}^{\textrm{I}}$, $c_{\textrm{unp},0}^{\textrm{I}}$ is kinematically suppressed in comparison to $c_{\textrm{unp},1}^{\textrm{I}}$, so $A_{\textrm{C}}^{\cos\phi}$ and $A_{\textrm{LU,I}}^{\sin\phi}$ are expected to have the largest magnitude in H{\sc ermes} kinematic conditions.

Although strictly dependent on higher-twist quantities, the asymmetry amplitudes $A_{\textrm{LU},\textrm{I}}^{\sin(2\phi)}$ and $A^{\cos(2\phi)}_{\textrm{C}}$ can also be expressed as having a dependence on $\mathcal{C}_{\textrm{unp}}^{\textrm{I}}$ using the Wandzura-Wilzcek approximation~\cite{Wan}. Neglecting antiquark-gluon-quark contributions, these higher-twist dependent amplitudes can be considered as being kinematically suppressed compared to the leading-twist amplitudes.
