...
production is \red{predominantly due to the failure to identify one photon from the decay of $\pi^0$ mesons}. The procedure for estimating the corresponding
...
refs.~\cite{Air08,Air09}. Each measurement in the $-t$, $x_{\textrm{B}}$ and
  ...
  centre of the relevant kinematic bin \red{and} included as part of the
  total systematic uncertainty.

\red{A} contribution to the systematic uncertainties of the
  measured amplitudes \red{arises} from shift\red{s in}  the
missing-mass distributions\red{. Such shifts appear in a comparison of} electron and positron data~\cite{Zei09,Bur10}. The missing-mass distribution is also adjusted for each calorimeter calibration period of data
collected in 2007 to account for an observed time dependence. One
...
The predominant contributio to the systematic uncertainty arises from detector
...
used for the simulation because it describes the data well and \red{was employed in r}ef.~\cite{Air09}. Asymmetry amplitudes were extracted
...
\red{calculated} from the GPD model at the average $-t$, $x_{\textrm{B}}$ and
...
independent scale uncertainty \red{without consideration in the other presented uncertainties.}
...
resonant state (``associated production''). The recoiling proton is not detected and
  the calorimeter resolution does not allow separation of the latter events from \red{the} rest of the data sample.
