...
construction of a longitudinal-momentum-dissected transverse spatial map of parton positions~\cite{Bur00}. \red{There have been several exclusive experimental results that can be used to constrain GPDs, (see \cite{Air00,Air00b,Air01,Air01a,Air02,Air03,Air04,Air06,Air08,Air08a,Air09,Air09a,Air09b,Air10,Air10a,Air10b,Air10c,Air11,Air11a,Air11b}). In particular, the exclusive leptoproduction of real photons from a nucleon or nucleus is the simplest to describe mathematically and is the most widely-used reaction channel for such work.}
...
the skewness $\xi$; and \red{$Q^2=-(q^2)$, i.e. }the negative square of the four-momentum of
...
emission of a \red{b}remsstrahlung photon by the lepton. \red{The leading terms in these processes are shown in figure~\ref{spin}.}
...
amplifies \red{the interference term, which is proporional to the DVCS amplitude. At H{\sc ermes} kinematic conditions, it is through the study of this interference term that useful information for the constraint of certain GPDs can be obtained~\cite{Bel02b}, especially since the interference term is the only part of the squared scattering amplitude that is linear in CFFs}.
...
explanation of the Fourier coefficients [$c_{\textrm{unp},n}^V,s_{\textrm{unp},n}^W$]
...
c_{\textrm{unp},n}^{\textrm{DVCS}}\cos(n\phi)} \textrm{\, and}&
\label{e:aludvcs}
\end{align}
...
\red{the differential} cross section with positive (negative) beam charge and
$\textrm{d}\sigma(\phi)^\rightarrow$ ($\textrm{d}\sigma(\phi)^\leftarrow$) refer\red{s}
to \red{the differential} cross section taken with beam spin parallel (anti-parallel) to the
beam momentum.

The $s_{\textrm{unp}\red{,n}}^{W}$ and $c_{\textrm{unp}\red{,n}}^{\textrm{\red{W}}}$ Fourier
coefficients depend on ``$\mathcal{C}$-functions'' \cite{Bel02b}, each of which is a
combination of complex\red{-valued} Compton Form Factors (CFFs). A CFF is a
\red{flavour-sum of} convolution\red{s} of GPD\red{s} with hard scattering kernel\red{s}. Contributions to the cross section \red{are} relatively suppressed by \red{factors that} may be kinematic in nature or due to the twist-level of the GPD\red{s appearing in that contribution}. Leading twist is twist-2. Typically, the contribution of a twist-$n$ GPD, and hence the corresponding CFF, is
suppressed by $\mathcal{O}(1/Q^{n-2})$. 

\red{The Fourier coefficients that receive leading-twist contributions are $c_{0,\textrm{unp}}^{\textrm{I}}$, $s_{1,\textrm{unp}}^{\textrm{I}}$ and $c_{1,\textrm{unp}}^{\textrm{I}}$. All} of these Fourier coefficients have a dominant contribution from the $\mathcal{C}_{\textrm{unp}}^{\textrm{I}}$-function:
\begin{eqnarray}
c_{0,\textrm{unp}}^{\textrm{I}} &\approx&-8(2-y)\frac{(2-y^2)}{(1-y)}K^2\mathfrak{Re}\mathcal{C}_{\textrm{unp}}^{\textrm{I}}\label{eq:c0}
\\
s_{1,\textrm{unp}}^{\textrm{I}} &\approx&\,8Ky(2-y)\mathfrak{Im}\mathcal{C}_{\textrm{unp}}^{\textrm{I}}\quad\textrm{and}\label{eq:s1}
\\
c_{1,\textrm{unp}}^{\textrm{I}} &\approx&\,8K(2- 2y + y^{2})\mathfrak{Re}\mathcal{C}_{\textrm{unp}}^{\textrm{I}}\,.\label{eq:c1}
\end{eqnarray}
\red{The definition of the kinematic factor \red{$K$ is~\cite{Bel02b}}
\begin{equation}
K=\sqrt{\frac{t}{Q^2}\Big(1-\frac{t_{\textrm{min}}}{t}\Big)\Big(1-x_{\textrm{B}}\Big)\Big(1-y-\frac{y^2\epsilon^2}{4}\Big)\Big\lbrace\sqrt{1+\epsilon^2}+\frac{4x_{\textrm{B}}(1-x_{\textrm{B}})+\epsilon^2}{4(1-x_{\textrm{B}})}
\frac{t_{\textrm{min}}-t}{Q^2}\Big\rbrace}\label{eq:K}
\end{equation} 
where $y$ is the fractional energy transfer from the beam lepton the virtual photon. The factor of $\Big(1-\frac{t_{\textrm{min}}}{t}\Big)$ implies that amplitudes proportional to these fourier coefficients vanish as $-t$ approaches its minimum value.}
...
neglected in eq.~\ref{Eq_cunp} with respect to $\mathcal{H}$ \red{(in first approximation)} since they
...
Compared to the analysis in ref.~\cite{Air09}, the analysis presented here additionally includes a larger, independent data set taken during the years 2006 and 2007 after the
experiment was \red{augmented} in the target region. The work covered
