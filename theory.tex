...
Leptoproduction of real photons \blue{($e\,p\,\rightarrow\,e\,p\,\gamma$)} can be described in terms of the two experimentally indistinguishable processes shown in Fig.~\ref{spin}: the Deeply Virtual Compton Scattering (DVCS) process, which is the
...
upgraded detector equipment \blue{(i.e. a missing-mass technique for event selection is used)}, but represent the largest DVCS data set that will
...
this paper \blue{(like} Ref.~\cite{Air09}\blue{)}, presents results on the following asymmetries:
\begin{align}
