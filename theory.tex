...
\caption[DVCS and Bethe Heitler hand bag diagram.]{\blue{(a)}: The DVCS process in
which an electron/positron (\blue{$e$}) interacts with a quark in the nucleon \ \blue{($p$)} via a virtual photon ($\gamma^\ast$). The quark is found in the nucleon with longitudinal momentum fraction $x+\xi$ and emits a real photon ($\gamma$). The quark is absorbed by the nucleon with fraction $x-\xi$. \blue{(b)}: The Bethe-Heitler process which has the same initial and final states as DVCS.}
\label{spin}
\end{center}
\end{figure}
...
useful information for the constraint of certain GPDs can be obtained \blue{\cite{Bel02b}}.
...
the nucleon mass\blue{,} and $\phi$ is the
...
beam-helicity asymmetries and beam-charge asymmetries. \blue{In order to disentangle the DVCS and interference contributions
...
c_{n}^{\textrm{DVCS}}\cos(n\phi)} \blue{\textrm{\,\,and}}&
\label{e:aludvcs}
\end{align}
...
to cross sections taken with beam spin parallel \blue{(anti-parallel)} \blue{to} the
beam momentum.
