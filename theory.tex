...
construction of a longitudinal-momentum-dissected transverse spatial map of parton positions. \red{There have been several exclusive experimental results that can be used to constrain GPDs, (see \cite{air09}). In particular, the exclusive leptoproduction of real photons from a nucleon or nucleus is the simplest to describe mathematically and is the most widely-used reaction channel for such work.}
...
to \red{differential} cross section taken with beam spin parallel (anti-parallel) to the
beam momentum.
...
\red{flavour-sum of} convolution\red{s} of GPD\red{s} with hard scattering kernel\red{s}. Contributions to the cross section may be relatively suppressed by powers of
$1/Q$, the origin of which may be kinematic in nature or due to the
twist-level of the GPD\red{s appearing in that contribution}. Leading twist is twist-2. Typically, the contribution of a twist-$n$ GPD, and hence the corresponding CFF, is
...
\frac{t_{\textrm{min}}-t}{Q^2}\Big\rbrace}
\end{equation} 
where $y$ is the fractional energy transfer from the beam lepton the virtual photon. The factor of $\Big(1-\frac{t_{\textrm{min}}}{t}\Big)$ implies that these amplitudes proportional to these fourier coefficients vanish as $-t$ approaches its minimum value.}
