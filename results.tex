...
\blue{The dash-dotted curve in Figs.~\ref{bsa_xbjrange} and~\ref{bca_xbjrange} shows the result of a calculation made from a hybrid quark-diquark model with a Regge term that is included in order to describe accurately parton distribution functions at low $x$ values~\cite{Liu11}. The model incorporates fits only to global deep inelastic scattering and elastic data (for the limits and moments of the underlying GPDs) and exclusive data from Jefferson Lab (to describe the skewness dependence). The model is trained in the kinematic domain of the Hall A and B experiments at Jefferson Lab and the extracted underlying physics quantities are used to calculate the asymmetries in the H{\sc ermes} kinematic region. The model describes the $t$-projections of the $A^{\sin\phi}_{\textrm{LU}}$ amplitude well, but the projections in the other kinematic variables are not well described.

The open crosses in  Figs.~\ref{bsa_xbjrange} and~\ref{bca_xbjrange} show calculations of the asymmetries using Compton Form Factors extracted in the VGG formalism from experimental data (including the H{\sc ermes} data from 1996-2005)~\cite{Gui11}. The extraction of the Compton Form Factors is nearly model independent, but cannot be extended byond the H{\sc ermes} kinematic domain. The presented data is well described by the calculations.}
